/**
 * Voice profile extractor — reads a corpus of writing and produces a stylometric profile
 */

const SUPPORTED_EXTENSIONS = new Set([".md", ".txt", ".mdx", ".eml"]);
...
export function collectFiles(corpusDir, specificFiles) {
  const files = [];

  if (specificFiles && specificFiles.length > 0) {
    for (const f of specificFiles) {
      if (existsSync(f)) files.push(f);
      else console.error(`  Warning: file not found: ${f}`);
    }
  } else if (corpusDir) {
...
function analyzeOpeningsClosings(paragraphs) {
...
export function buildProfile(files) {
  let allText = "";
  let allRaw = "";
  const sources = [];
  ...
  const profile = {
    ...
    scoring: {
      ...
      sentenceLength: { target: { min: 0, max: 0 } },
      paragraphLength: { maxSentences: 0 },
